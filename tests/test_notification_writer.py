...
@pytest.mark.asyncio
async def test_notification_writer_waits_for_inflight_playing_before_cancelled() -> None:
...
@pytest.mark.asyncio
async def test_notification_writer_waits_for_inflight_playing_before_flush_ack() -> None:
    # Given: a playing notification whose connection send is already in flight.
    sender = _BlockedSender()
    writer = NotificationWriter(sender)
    async with asyncio.TaskGroup() as task_group:
