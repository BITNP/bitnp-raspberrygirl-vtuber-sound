...
def test_playback_service_emits_monotonic_queue_start_progress_done_events() -> None:
    ...
    assert [event.segment_id for event in sink.events] == [SegmentId("seg-sound-001")] * 4
    assert [event.command_id for event in sink.events] == ["sound-001"] * 4
    assert service.queue_depth == 0


def test_playback_service_cancel_mid_playback_clears_queue_and_emits_cancelled() -> None:
    # Given: one active segment and one queued segment.
    clock = ManualPlaybackClock()
    sink = RecordingSink()
    service = PlaybackService(clock=clock, sink=sink)
    service.enqueue(parse_play_command(play_envelope("seg-sound-001", "sound-001", 200)))
    service.enqueue(parse_play_command(play_envelope("seg-sound-002", "sound-002", 200)))
    clock.advance(75)
    service.tick()
