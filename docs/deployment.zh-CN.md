...
| `SOUND_RTP_STREAM_ID` | Orchestrator 命令的稳定 sink 标识。 |
...
| `SOUND_TRACE_ID`、`SOUND_SESSION_ID` | 可选的注册 envelope 标识。两者默认都是 `sound-receive`。 |
...
使用 `uv run sound-receive` 启动运行时。它在建立 WSS 连接前绑定 UDP，发送 `media.rtp.sink.register`，然后等待命令。有效命令必须使用 L16、16 kHz、单声道、负载类型 96、每帧 320 个采样、已注册端点及其 SSRC。
