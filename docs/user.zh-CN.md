...
部署时配置 `.env.example` 中的 `ORCHESTRATOR_WS_URL`、`TRUSTED_LAN_TOKEN`、`SOUND_SESSION_ID`、`SOUND_RTP_STREAM_ID`、`SOUND_RTP_BIND_PORT` 和 `SOUND_RTP_ADVERTISED_HOST`。生产环境必须使用 WSS 和 token。
