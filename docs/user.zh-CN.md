...
现场讲解链路中，Sound 应在 Orchestrator 后、Mic 前启动。它接收的是 Orchestrator 生成的 RTP，不是 Mic 原始 RTP。只有实际消费完当前流后才发送 `finished`；该事件携带当前媒体关联信息，是 Orchestrator 释放输出 lease、允许下一轮播放的依据。
